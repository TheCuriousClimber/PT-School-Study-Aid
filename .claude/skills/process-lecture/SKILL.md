...
description: Turn a week's lecture files plus the professor's learning objectives into a blueprint, a budgeted flashcard deck, practical prep, an exam simulation, a time-boxed study plan, and a five-minute clinical primer, following prompts/project-instructions.md. Use when the user says "process", names a course folder under courses/, or drops lecture PDFs or PowerPoints in the repo.
---
...
4. Run the `Process` pipeline from the instructions, Parts 0 through 6, and write the whole output to `study-guide.md` inside the week folder. Do not print it to the chat as well.
